...
import SupabaseFileUtils from "src/infra/file/supabase-file.manager"
...
describe("FeedService", () => {
  ...
  it("post", async () => {
    ...
    jest
      .spyOn(addressResolver, "resolveAddress")
      .mockReturnValueOnce(
        new Promise((resolve, reject) => resolve("MOCK_ADDRESS")),
      )
    jest
      .spyOn(addressResolver, "resolveCoord")
      .mockReturnValueOnce(
        new Promise((resolve, reject) => resolve({ lng: 10, lat: 10 })),
      )
    jest
      .spyOn(feedRepository, "create")
      .mockReturnValueOnce(new Promise((resolve, reject) => resolve(feed)))

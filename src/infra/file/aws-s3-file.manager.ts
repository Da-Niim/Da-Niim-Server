...
export class AWSS3FileManager implements FileManager {
    ...
    constructor(@Inject(s3Config.KEY) private readonly _s3Config: ConfigType<typeof s3Config>) {  
    ...
    async getPresignedUrl(filename: string): Promise<{presignedUrl: string, key: string}> {
      let mimeType = mime.lookup(this.parseExt(filename))

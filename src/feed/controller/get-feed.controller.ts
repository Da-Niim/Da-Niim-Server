...
import { GetFeedService } from "../application/get-feed.service"
  ...
  export class GetFeedController {
    ...
    async getFeeds(@Query() query: GetFeedRequest, @Req() req: Request): Promise<PaginationResponse<GetFeedResponse[]>> {
      return await this.getFeedService.getFeeds(await query.toCommand(req.user._id))
    }
    ...
    async getProfileFeeds(@Query() query: GetProfileFeedRequest, @Req() req: Request): Promise<PaginationResponse<GetProfileFeedResponse[]>> {
      const user = req.user
      return await this.getFeedService.getProfileFeeds(await query.toCommand(user._id))
    }
  }